...
export const deleteApi = async (id: number): Promise<{ code: number; msg: string; data?: unknown }> => {
  try {
    ...
    // 直接检查 response 的 code，保留后端返回的原始错误码
    ...
    if ((error as { response?: Record<string, unknown> }).response) {
      ...
      if (responseData?.message || responseData?.msg) {
        errorMsg = responseData.message || responseData.msg || '删除失败'
...
// 导出学生数据 API
export const exportStudentDataApi = async (params: Record<string, unknown> = {}) => {
  try {
