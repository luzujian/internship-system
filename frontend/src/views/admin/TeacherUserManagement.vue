...
const downloadTemplate = async (): Promise<void> => {
  try {
    // 响应拦截器已返回 Blob
    const blob = await request.get('/admin/teachers/import-template', {
      responseType: 'blob'
    })
...
const handleExportToExcel = async () => {
  try {
    ...
    // 响应拦截器已返回 Blob
    const blob = await exportTeacherDataApi()
    ...
    <el-card class="actions-card" shadow="never">
      <div class="actions-container">
        ...
        <div class="secondary-actions">
          ...
          <el-button v-if="authStore.hasPermission('user:teacher:view')" type="success" @click="handleExportToExcel" class="action-btn success">
    ...
    <el-dialog v-model="dialogFormVisible" :title="formTitle" width="500px" @close="handleCancel()" class="form-dialog">
      ...
      <template v-if="formTitle === '新增教师'">
        ...
        >
          ...
          <el-form-item label="手机号" prop="phone">
            <el-input v-model="addTeacher.phone" placeholder="请输入手机号（非必填）" />
          </el-form-item>
          <div class="form-tip">
      ...
      <template v-else-if="formTitle === '编辑教师'">
        ...
        >
          ...
          <el-form-item label="手机号" prop="phone">
            <el-input v-model="editTeacher.phone" placeholder="请输入手机号（非必填）" />
          </el-form-item>
          <div class="form-tip">
            提示：密码将自动设置为123456，无需手动输入
          </div>
