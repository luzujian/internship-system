...
import StudentUserService, { queryPageApi, addApi, queryInfoApi, updateApi, deleteApi, resetPasswordApi, batchDeleteApi, importExcelApi, updateStatusApi, exportStudentDataApi } from '../../api/StudentUserService'
...
const formatDepartmentName = (_row: unknown, _column: unknown, cellValue: string): string => {
  if (!cellValue) return ''

  // 从departments数组中查找对应的院系名称
  const dept = departments.value.find(item => item.value === String(cellValue))
  return dept ? dept.name : String(cellValue)
}

// 搜索表单对象
let searchForm = ref<Partial<StudentUser>>({ studentId: '', name: '', gender: '', grade: '', majorId: '', classId: '', status: '', department: '' })
...
// 院系列表数据
let departments = ref<{ value: string; name: string }[]>([])
...
// 是否启用级联删除（暂时禁用，等待完整实现）
let enableCascadeDelete = false
  ...
  try {
    ...
    if (response && response.code === 200 && response.data) {
      ...
      const enhancedData = dataList.map(student => {
        ...
        //找到对应的院系，确保类型一致
        const dept = departments.value.find(d => d.value === String(student.department))
        return {
          ...student,
          majorName: major ? major.name : '未知专业',
          className: classItem ? classItem.name : '未知班级',
          departmentName: dept ? dept.name : '未知院系',
          // 确保日期字段名称与表格中使用的一致
...
const handleDepartmentChange = (value) => {
  // 院系变化时清除专业和班级选择
  searchForm.value.department = value
  searchForm.value.majorId = null
  searchForm.value.classId = null
}

// 处理对话框中院系选择变化事件
const handleDialogDepartmentChange = (value) => {
  // 院系变化时清除专业和班级选择
  if (formTitle.value === '新增学生') {
    addStudent.value.department = value
    addStudent.value.majorId = null
    addStudent.value.classId = null
  } else {
    editStudent.value.department = value
    editStudent.value.majorId = null
    editStudent.value.classId = null
  }
}
...
// 根据院系过滤专业的计算属性
const filteredMajorsForAdd = computed(() => {
  if (!addStudent.value.department) return majors.value
  return majors.value.filter(m => String(m.departmentId) === String(addStudent.value.department))
})

const filteredMajorsForEdit = computed(() => {
  if (!editStudent.value.department) return majors.value
  return majors.value.filter(m => String(m.departmentId) === String(editStudent.value.department))
})

const filteredMajorsForSearch = computed(() => {
  if (!searchForm.value.department) return majors.value
  return majors.value.filter(m => String(m.departmentId) === String(searchForm.value.department))
})
...
const getDepartmentList = async () => {
  try {
    ...
    departments.value = departmentData
      .filter(dept => dept && (dept.id || dept.departmentId))
      .map(dept => {
        const deptId = String(dept.id || dept.departmentId || '')
        const deptName = dept.name || dept.departmentName || '未知院系'
        return {
          value: deptId,
          name: deptName
        }
    ...
    logger.log('处理后的院系列表:', departments.value)
  } catch (error) {
...
const getMajorList = async () => {
  try {
    ...
    if (cached && cachedTime) {
      ...
      // 检查缓存数据是否有 departmentId 字段，如果没有则重新获取
      const cachedMajors = JSON.parse(cached)
      const hasDepartmentId = cachedMajors.length > 0 && cachedMajors[0].departmentId !== undefined
      if (age < 5 * 60 * 1000 && hasDepartmentId) {
    ...
    majors.value = rawMajors.map(major => {
      ...
      // 提取院系ID，支持多种属性名
      const deptId = major.departmentId || major.department_id || major.DepartmentId || major.DEPARTMENT_ID || ''
      return {
...
const downloadTemplate = async (): Promise<void> => {
  try {
    // 响应拦截器已返回 Blob
    const blob = await request.get('/admin/students/import-template', {
      responseType: 'blob'
    })
...
const handleExportToExcel = async () => {
  try {
    ...
    // 响应拦截器已返回 Blob
    const blob = await exportStudentDataApi()
      ...
      <el-form ref="searchFormRef" :inline="true" :model="searchForm" class="search-form" @keyup.enter="queryPage">
        ...
        <div class="search-row">
          <el-form-item label="院系">
            <el-select v-model="searchForm.department" placeholder="请选择院系" clearable filterable @change="handleDepartmentChange">
              <el-option v-for="dept in departments" :key="dept.value" :label="dept.name" :value="dept.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="专业">
            <el-select v-model="searchForm.majorId" placeholder="请选择专业" clearable filterable @change="handleMajorChange">
              <el-option v-for="major in filteredMajorsForSearch" :key="major.value" :label="major.name" :value="major.value"></el-option>
            </el-select>
          </el-form-item>
    ...
    <el-card class="actions-card" shadow="never">
      <div class="actions-container">
        ...
        <div class="secondary-actions">
          ...
          <el-button v-if="authStore.hasPermission('user:student:view')" type="success" @click="handleExportToExcel" class="action-btn success">
    ...
    <el-dialog v-model="dialogFormVisible" :title="formTitle" width="500px" @close="handleCancel()" class="form-dialog">
      ...
      <template v-if="formTitle === '新增学生'">
        ...
        >
          ...
          <el-form-item label="院系" prop="department">
            <el-select v-model="addStudent.department" placeholder="请选择院系" filterable @change="handleDialogDepartmentChange">
              <el-option v-for="dept in departments" :key="dept.value" :label="dept.name" :value="dept.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="专业" prop="majorId">
            <el-select v-model="addStudent.majorId" placeholder="请选择专业" @change="handleDialogMajorChange">
              <el-option v-for="major in filteredMajorsForAdd" :key="major.value" :label="major.name" :value="major.value"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="手机号" prop="phone">
            <el-input v-model="addStudent.phone" placeholder="请输入手机号"></el-input>
          </el-form-item>
          <div class="form-tip">
      ...
      <template v-else-if="formTitle === '编辑学生'">
        ...
        >
          ...
          <el-form-item label="院系" prop="department">
            <el-select v-model="editStudent.department" placeholder="请选择院系" filterable @change="handleDialogDepartmentChange">
              <el-option v-for="dept in departments" :key="dept.value" :label="dept.name" :value="dept.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="专业" prop="majorId">
            <el-select v-model="editStudent.majorId" placeholder="请选择专业" @change="handleDialogMajorChange">
              <el-option v-for="major in filteredMajorsForEdit" :key="major.value" :label="major.name" :value="major.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="班级" prop="classId">
          ...
          <el-form-item label="手机号" prop="phone">
            <el-input v-model="editStudent.phone" placeholder="请输入手机号"></el-input>
          </el-form-item>
          <el-form-item label="邮箱" prop="email">
            <el-input v-model="editStudent.email" placeholder="请输入邮箱"></el-input>
          </el-form-item>
          <div class="form-tip">
            提示：密码将自动设置为123456，无需手动输入
          </div>
