...
const handleExportToExcel = async (): Promise<void> => {
  try {
    ...
    const blob = await DepartmentService.exportMajorData()
    const url = window.URL.createObjectURL(blob as unknown as Blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `院系专业数据_${new Date().getTime()}.xlsx`
...
const downloadTemplate = async (): Promise<void> => {
  try {
    // 响应拦截器已返回 Blob
    const blob = await request.get('/admin/departments/majors/import-template', {
      responseType: 'blob'
    })
