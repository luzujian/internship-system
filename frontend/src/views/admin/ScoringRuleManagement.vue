...
const availableCategories = computed(() => {
  // 基于category_weight中已存在的分类来过滤，不显示已有分类
  const existingCategoryCodes = new Set(Object.keys(categoryWeights.value))
  return allCategories.filter(cat => !existingCategoryCodes.has(cat.value))
})
...
const handleDeleteCategory = async (categoryName): Promise<void> => {
  try {
    ...
    const response = await request.delete(`/admin/scoring-rule/category/${categoryName}`)
    if (response.code === 200) {
...
const handleSave = async (): Promise<void> => {
  ...
  try {
    ...
    if (form.value.id) {
      ...
      ElMessage.info('AI正在生成评分规则描述，请稍候（可能需要10-20秒）...')
      response = await request.post('/admin/scoring-rule/batch-create-with-ai', { categoryName: form.value.category })
    }
...
const redistributeWeights = async (): Promise<void> => {
  try {
    const response = await request.post('/admin/scoring-rule/redistribute-weights')
    if (response.code === 200) {
      await fetchCategoryWeights()
    } else {
