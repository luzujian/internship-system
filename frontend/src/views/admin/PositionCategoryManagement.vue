...
const downloadTemplate = async (): Promise<void> => {
  try {
    // 响应拦截器已返回 Blob
    const blob = await request.get('/admin/position-categories/import-template', {
      responseType: 'blob'
    })
...
const handleExportToExcel = async (): Promise<void> => {
  try {
    ...
    const blob = await PositionCategoryService.exportCategoryData()
    const url = window.URL.createObjectURL(blob as unknown as Blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `岗位类别数据_${new Date().getTime()}.xlsx`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    window.URL.revokeObjectURL(url)
