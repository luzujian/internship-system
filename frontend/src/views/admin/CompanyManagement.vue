...
const queryPage = async (): Promise<void> => {
  ...
  try {
    ...
    const response = await companyService.getAdminCompanies(params)
...
const handleExportToExcel = async (): Promise<void> => {
  try {
    ...
    // 响应拦截器已返回 Blob
    const blob = await companyService.exportCompanies(params)
...
const downloadTemplate = async (): Promise<void> => {
  try {
    // 响应拦截器已返回 Blob
    const blob = await request.get('/admin/companies/import-template', {
      responseType: 'blob'
    })
    ...
    <el-card class="actions-card" shadow="never">
      <div class="actions-container">
        ...
        <div class="secondary-actions">
          ...
          <el-button v-if="authStore.hasPermission('user:company:view')" type="success" @click="handleExportToExcel" class="action-btn success">
