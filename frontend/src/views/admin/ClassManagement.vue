...
//获取辅导员列表（只获取身份为辅导员的教师）
const getTeacherList = async (): Promise<void> => {
  try {
    logger.log('开始获取辅导员列表...')
    const response = await TeacherUserService.getAllCounselors()
    logger.log('辅导员列表响应:', response)
    ...
    teacherList.value = teachers.filter((t: any) => {
      ...
      name: String(t.name || ''),
      teacherType: t.teacherType || 'COUNSELOR'
    }))

    logger.log('最终的辅导员列表数据:', teacherList.value)
  } catch (error) {
    logger.error('获取辅导员列表失败:', error)
    teacherList.value = []
    ElMessage.error('获取辅导员列表失败：' + (error instanceof Error ? error.message : '未知错误'))
  }
}
...
const handleUpdateClass = async (id: string): Promise<void> => {
  ...
  if(result.code === 200 && result.data){
    ...
    editClass.value = {
      ...
      majorId: classData.majorId ? String(classData.majorId) : null,
...
const handleExportToExcel = async (): Promise<void> => {
  try {
    ...
    // 使用 toRaw 处理响应式代理，确保获取原始数组
    const rawTableData = toRaw(tableData.value)
    const allClasses = Array.isArray(rawTableData) ? rawTableData : []
...
const downloadTemplate = async (): Promise<void> => {
  try {
    ...
    // 响应拦截器已返回 response.data (Blob)
    const blob = await downloadClassTemplateApi()
  ...
  <div class="class-management-container">
    ...
    <el-card class="actions-card" shadow="never">
      <div class="actions-container">
        ...
        <div class="secondary-actions">
          ...
          <el-button v-if="authStore.hasPermission('class:view')" type="success" @click="handleExportToExcel" class="action-btn success">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="majorName" label="专业" align="center" width="220" />
