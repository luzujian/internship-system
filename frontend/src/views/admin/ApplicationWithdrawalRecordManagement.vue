    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <template v-if="activeTab === 'student'">
          ...
          <el-table-column label="实习状态" width="150" align="center">
...
const handleExport = async (): Promise<void> => {
  try {
    ...
    if (activeTab.value === 'student') {
      ...
      if (searchForm.applicantName) params.append('name', searchForm.applicantName)
      const url = `/api/admin/internship-status/recall/export?${params.toString()}`
      ...
      link.download = `学生撤回申请记录_${new Date().getTime()}.xlsx`
      ...
      const params = new URLSearchParams()
      if (searchForm.companyName) params.append('companyName', searchForm.companyName)
      if (searchForm.contactPerson) params.append('contactPerson', searchForm.contactPerson)
      if (searchForm.withdrawalTimeRange && searchForm.withdrawalTimeRange[0]) {
        params.append('startTime', searchForm.withdrawalTimeRange[0])
        params.append('endTime', searchForm.withdrawalTimeRange[1])
      }
      const url = `/api/admin/companies/recall-records/export?${params.toString()}`
      const response = await fetch(url, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${accessToken}`
      ...
      const link = document.createElement('a')
      link.href = downloadUrl
      link.download = `企业撤回申请记录_${new Date().getTime()}.xlsx`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      window.URL.revokeObjectURL(downloadUrl)
    ...
    ElMessage.error('导出Excel失败: ' + (error instanceof Error ? error.message : '未知错误'))
  }
}
