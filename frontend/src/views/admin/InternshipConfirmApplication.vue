...
const exportAll = async (): Promise<void> => {
  try {
    ...
    if (response.code === 200 && response.data) {
      ...
      const exportData = data.map((item: InternshipStatusData) => ({
        '学号': item.student?.studentUserId || '',
        '学生姓名': item.student?.name || '',
        '性别': formatGender(null, null, item.student?.gender ?? 0),
        '实习单位': item.company?.companyName || '',
        '岗位名称': item.position?.positionName || '',
        '实习状态': formatStatus(null, null, item.status),
        '企业确认状态': formatCompanyConfirmStatus(null, null, item.companyConfirmStatus),
        '创建时间': formatDate(null, null, item.createTime),
        '更新时间': formatDate(null, null, item.updateTime),
        '实习开始时间': formatDate(null, null, item.internshipStartTime ?? ''),
        '实习结束时间': formatDate(null, null, item.internshipEndTime ?? ''),
        '实习时长（天）': item.internshipDuration ?? '',
        '反馈': item.feedback ?? '',
        '备注': item.remark ?? ''
      }))
        ...
        <div class="search-row">
          ...
          <el-form-item label="实习状态">
            <el-select v-model="searchForm.status" placeholder="请选择实习状态" clearable>
              <el-option label="未找到实习" :value="0"></el-option>
              <el-option label="有 Offer 未确定" :value="1"></el-option>
              <el-option label="已确定实习" :value="2"></el-option>
              <el-option label="实习结束" :value="3"></el-option>
            </el-select>
          </el-form-item>
      ...
      <el-scrollbar height="600px">
        <div v-if="currentRow" class="detail-content">
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="实习单位" :span="2">{{ currentRow.company?.companyName || '' }}</el-descriptions-item>
            <el-descriptions-item label="岗位名称" :span="2">{{ currentRow.position?.positionName || '' }}</el-descriptions-item>
