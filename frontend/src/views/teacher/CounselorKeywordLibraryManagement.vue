  ...
  <div class="keyword-library-container">
    <div class="page-header">
      <div class="header-content">
        ...
        <p class="page-description">管理AI分析的关键词库，用于识别学生实习表现</p>
      </div>
...
const fetchKeywords = async () => {
  loading.value = true
  try {
    const response = await request.get(`/counselor/keyword-library/${counselorId.value}`)
...
const handleFilter = () => {
  if (filterStatus.value === '') {
    ...
    keywordList.value = keywordList.value.filter(k => k.status === filterStatus.value)
  }
}
...
const handleDelete = async (row: any) => {
  try {
    ...
    const response = await request.delete(`/counselor/keyword-library/${row.id}`)
    if (response.code === 200) {
...
const handleToggleStatus = async (row: any) => {
  ...
  try {
    const response = await request.put(`/counselor/keyword-library/${row.id}`, {
      id: row.id,
      keyword: row.keyword,
      status: newStatus
    })
...
const handleSave = async () => {
  ...
  try {
    let response
    if (form.value.id) {
      response = await request.put(`/counselor/keyword-library/${form.value.id}`, {
        ...form.value,
        counselorId: counselorId.value
      })
    } else {
      response = await request.post('/counselor/keyword-library', {
        ...form.value,
        counselorId: counselorId.value
      })
    }
