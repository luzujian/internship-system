  ...
  actions: {
    ...
    handleLoginSuccess(result: LoginResponse['data'], userType: string): boolean {
      ...
      if (loginResult.token) {
        ...
        userInfo = loginResult.userInfo || loginResult.user as User & { role: string; teacherType?: string }
        role = userInfo.role
        ...
        refreshToken = loginResult.refreshToken || null
        userInfo = loginResult.user as User & { role: string; teacherType?: string }
        role = userInfo.role
        teacherType = userInfo.teacherType
        permissions = loginResult.permissions
      ...
      if (restoreResult.token) {
        ...
        if (decoded) {
          ...
          this.user = {
            ...
            teacherType: localStorage.getItem('teacherType') || undefined,
            isReadOnly: restoreResult.isReadOnly || false,
            originalAdminUsername: restoreResult.originalAdminUsername || null,
            // 学生用户字段（从 localStorage 恢复）
            studentId: localStorage.getItem('student_' + 'studentId_' + restoreResult.role) || localStorage.getItem(STORAGE_KEYS.STUDENT_ID),
            school: localStorage.getItem('student_' + 'school_' + restoreResult.role),
            department: localStorage.getItem('student_' + 'department_' + restoreResult.role),
            major: localStorage.getItem('student_' + 'major_' + restoreResult.role),
            class: localStorage.getItem('student_' + 'class_' + restoreResult.role),
            gender: localStorage.getItem('student_' + 'gender_' + restoreResult.role),
            phone: localStorage.getItem('student_' + 'phone_' + restoreResult.role),
            email: localStorage.getItem('student_' + 'email_' + restoreResult.role),
            grade: localStorage.getItem('student_' + 'grade_' + restoreResult.role)
          }
        }
    ...
    buildUserFromStorage(role: string, rolePrefix: string) {
      return {
        ...
        // 学生用户字段
        studentId: localStorage.getItem(rolePrefix + 'studentId_' + role),
        school: localStorage.getItem(rolePrefix + 'school_' + role),
        department: localStorage.getItem(rolePrefix + 'department_' + role),
        major: localStorage.getItem(rolePrefix + 'major_' + role),
        class: localStorage.getItem(rolePrefix + 'class_' + role),
        gender: localStorage.getItem(rolePrefix + 'gender_' + role),
        phone: localStorage.getItem(rolePrefix + 'phone_' + role),
        email: localStorage.getItem(rolePrefix + 'email_' + role),
        grade: localStorage.getItem(rolePrefix + 'grade_' + role)
      }
    },
    ...
    saveExtraInfo(
      role: string,
      userInfo: User & { role: string; teacherType?: string; studentId?: string; school?: string; department?: string; major?: string; class?: string; gender?: number; phone?: string; email?: string; grade?: string },
      ...
      // 保存学生用户特有字段到 localStorage
      if (role === CONFIG.ROLES.ROLE_STUDENT) {
        const rolePrefix = getRoleStoragePrefix(role)
        if (userInfo.studentId) localStorage.setItem(rolePrefix + 'studentId_' + role, userInfo.studentId)
        if (userInfo.school) localStorage.setItem(rolePrefix + 'school_' + role, userInfo.school)
        if (userInfo.department) localStorage.setItem(rolePrefix + 'department_' + role, userInfo.department)
        if (userInfo.major) localStorage.setItem(rolePrefix + 'major_' + role, userInfo.major)
        if (userInfo.class) localStorage.setItem(rolePrefix + 'class_' + role, userInfo.class)
        if (userInfo.gender) localStorage.setItem(rolePrefix + 'gender_' + role, String(userInfo.gender))
        if (userInfo.phone) localStorage.setItem(rolePrefix + 'phone_' + role, userInfo.phone)
        if (userInfo.email) localStorage.setItem(rolePrefix + 'email_' + role, userInfo.email)
        if (userInfo.grade) localStorage.setItem(rolePrefix + 'grade_' + role, userInfo.grade)
        logger.log('已保存学生用户额外信息到本地存储')
      }
    ...
    updateAuthState(accessToken: string, userInfo: User & { role: string; teacherType?: string; studentId?: string; school?: string; department?: string; major?: string; class?: string; gender?: number; phone?: string; email?: string; grade?: string }, role: string, refreshToken: string | null) {
      this.setToken(accessToken)
      this.user = {
        id: String(userInfo.id),
