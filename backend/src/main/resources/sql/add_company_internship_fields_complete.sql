...
-- 添加企业标签字段（系统自动标识：双向选择阶段/学生自主联系/接受兜底）
ALTER TABLE company_users ADD COLUMN company_tag VARCHAR(50) COMMENT '企业标签：双向选择阶段/学生自主联系/接受兜底' AFTER max_backup_students;

-- 添加注册时间字段（用于时间线判断）
ALTER TABLE company_users ADD COLUMN register_time DATETIME COMMENT '注册时间（用于时间线判断）' AFTER company_tag;

-- 为system_settings表添加双向选择时间段字段
ALTER TABLE system_settings ADD COLUMN dual_selection_start_date DATE COMMENT '双向选择阶段开始日期' AFTER session_timeout;

ALTER TABLE system_settings ADD COLUMN dual_selection_end_date DATE COMMENT '双向选择阶段结束日期' AFTER dual_selection_start_date;
