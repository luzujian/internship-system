...
public class AdminPermissionServiceImpl implements AdminPermissionService {
    ...
    public List<Map<String, Object>> getPermissionTree() {
        ...
        if (mainModuleMap.containsKey(targetModule)) {
            log.info("撤回申请记录管理 包含的权限数量：{}", mainModuleMap.get(targetModule).size());
            for (Permission p : mainModuleMap.get(targetModule)) {
                log.info("撤回申请记录管理 - 权限：{}, 模块：{}", p.getPermissionCode(), p.getModule());
            }
        } else {
            log.warn("撤回申请记录管理 主模块不存在于 mainModuleMap 中");
            // 尝试查找相似的主模块
            for (String key : mainModuleMap.keySet()) {
