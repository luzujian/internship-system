...
import com.internship.mapper.GroupMapper;
...
import com.internship.service.ClassService;
import com.internship.service.DepartmentService;
import com.internship.service.MajorService;
import com.internship.service.StudentUserService;
import com.internship.vo.StudentUserVO;
...
public class StudentUserServiceImpl implements StudentUserService {
    ...
    public int register(StudentUser studentUser) {
        ...
        try {
            ...
            internshipStatus.setStudentId(studentUser.getId());
            internshipStatus.setStatus(0); // 默认状态：无offer
            internshipStatus.setCompanyConfirmStatus(0);
            internshipStatus.setHasComplaint(false);
            internshipStatus.setIsDelayed(false);
            internshipStatus.setIsInterrupted(false);
            internshipStatus.setCreateTime(new Date());
            internshipStatus.setUpdateTime(new Date());

            studentInternshipStatusMapper.insert(internshipStatus);
            log.info("学生实习状态记录创建成功，学号: {}", studentUser.getStudentId());
        } catch (Exception e) {
            log.error("创建学生实习状态记录失败，学号: {}, 错误: {}", studentUser.getStudentId(), e.getMessage());
            // 不影响学生注册成功，只记录错误日志
        }
    ...
    public StudentUser findById(Long id) {
        ...
        if (studentUser != null && studentUser.getClassId() != null) {
            Class classEntity = classService.findById(studentUser.getClassId());
            if (classEntity != null) {
                studentUser.setClassName(classEntity.getName());
                studentUser.setClasses(classEntity.getName());
            }
        }
        return studentUser;
    }
    ...
    @Override
    public PageResult<StudentUser> findPage(Integer page, Integer pageSize, String studentId, String name, Integer grade, String majorId, String classId, Integer status, Integer gender, String department) {
        log.debug("分页查询学生用户(含状态和性别), 页码: {}, 每页条数: {}, 学号: {}, 姓名: {}, 年级: {}, 专业ID: {}, 班级ID: {}, 状态: {}, 性别: {}, 院系: {}",
        ...
        List<StudentUser> studentUsers = studentUserMapper.listWithStatus(studentId, name, grade, majorIdInt, classIdValue != null ? classIdValue.toString() : null, status, gender, department);
    ...
    public PageResult<StudentUser> findPage(Integer page, Integer pageSize, String studentId, String name, Integer grade, String majorId, String classId, Integer status) {
        ...
        List<StudentUser> studentUsers = studentUserMapper.listWithStatus(studentId, name, grade, majorIdInt, classIdValue != null ? classIdValue.toString() : null, status, null, null);
    ...
    public Map<String, Object> importFromExcel(List<Map<String, Object>> studentDataList) {
        ...
        if (!validStudentList.isEmpty()) {
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                try {
                    // 重新查询刚插入的学生以获取ID
                    List<String> studentIdsInBatch = batchList.stream()
                            .map(StudentUser::getStudentId)
                            .toList();
                    List<StudentUser> insertedStudents = studentUserMapper.findByStudentIds(studentIdsInBatch);

                    Date now = new Date();
                    for (StudentUser insertedStudent : insertedStudents) {
                        try {
                            StudentInternshipStatus internshipStatus = new StudentInternshipStatus();
                            internshipStatus.setStudentId(insertedStudent.getId());
                            internshipStatus.setStatus(0); // 默认状态：无offer
                            internshipStatus.setCompanyConfirmStatus(0);
                            internshipStatus.setHasComplaint(false);
                            internshipStatus.setIsDelayed(false);
                            internshipStatus.setIsInterrupted(false);
                            internshipStatus.setCreateTime(now);
                            internshipStatus.setUpdateTime(now);

                            studentInternshipStatusMapper.insert(internshipStatus);
                            log.debug("批量导入学生实习状态记录创建成功，学号: {}", insertedStudent.getStudentId());
                        } catch (Exception e) {
                            log.error("批量导入创建学生实习状态记录失败，学号: {}, 错误: {}", insertedStudent.getStudentId(), e.getMessage());
                        }
                    }
                } catch (Exception e) {
                    log.error("批量创建实习状态记录失败，批次: {}, 错误: {}", i + 1, e.getMessage());
                }
            }
        }
