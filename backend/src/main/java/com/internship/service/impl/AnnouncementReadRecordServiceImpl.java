...
import com.internship.service.AnnouncementReadRecordService;
...
public class AnnouncementReadRecordServiceImpl implements AnnouncementReadRecordService {
    ...
    @Override
    public Long countUnreadByAnnouncementId(Long announcementId, String targetType, String targetValue) {
        log.debug("查询公告的未读人数，公告ID: {}, 目标类型: {}, 目标值: {}", announcementId, targetType, targetValue);

        if (announcementId == null || announcementId <= 0) {
            throw new BusinessException("公告ID无效");
        }

        return announcementReadRecordMapper.countUnreadByAnnouncementId(announcementId, targetType, targetValue);
    }
    ...
    public List<AnnouncementReadRecord> findByUserId(String userId, String userType) {
        log.debug("根据用户ID查询阅读记录列表，用户ID: {}, 用户类型: {}", userId, userType);
        ...
        List<AnnouncementReadRecord> records = announcementReadRecordMapper.findByUserId(userId, userType);
        return records != null ? records : Collections.emptyList();
    }
