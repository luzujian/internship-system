...
import com.internship.entity.LearningResource;
import com.internship.entity.ResourceCategory;
import com.internship.mapper.LearningResourceMapper;
import com.internship.mapper.ResourceCategoryMapper;
import com.internship.service.ResourceService;
...
public class ResourceServiceImpl implements ResourceService {
    ...
    public void reviewResource(Long resourceId, Boolean approved, Long reviewerId) {
        try {
            ...
            resource.setStatus(approved ? "PUBLISHED" : "REJECTED");
    ...
    @Override
    public Object getApprovedResources(Integer page, Integer pageSize) {
        logger.info("获取已审核资源列表（分页）: page={}, pageSize={}", page, pageSize);
        ...
        try {
            ...
            // 获取所有已审核资源
            List<LearningResource> allResources = learningResourceMapper.selectByStatus("APPROVED");
            
            // 处理资源类型
            allResources = processResourceTypes(allResources);
            
            // 构建分页结果
            return buildPagedResult(allResources, page, pageSize);
        } catch (Exception e) {
            logger.error("获取已审核资源列表失败: {}", e.getMessage(), e);
            // 返回空结果
    ...
    @Override
    public Object getPublishedResources(Integer page, Integer pageSize) {
        logger.info("获取已发布资源列表（分页）: page={}, pageSize={}", page, pageSize);
        ...
        try {
            ...
            // 获取所有已发布资源
            List<LearningResource> allResources = learningResourceMapper.selectByStatus("PUBLISHED");
            ...
            logger.error("获取已发布资源列表失败: {}", e.getMessage(), e);
            // 返回空结果
            Map<String, Object> errorResult = new HashMap<>();
            errorResult.put("list", new ArrayList<>());
            errorResult.put("total", 0);
    ...
    public Object aiSmartSearch(String query, String fileType, int page, int pageSize) {
        ...
        try {
            ...
            if (query != null && !query.trim().isEmpty()) {
                ...
                try {
                    ...
                    List<Map<String, Object>> searchResults = resources.stream()
                            .map(resource -> {
                                ...
                                resourceMap.put("uploaderRole", resource.getUploaderRole()); // 使用uploaderRole
                                resourceMap.put("downloadCount", resource.getDownloadCount() != null ? resource.getDownloadCount() : 0); // 使用实际的下载次数
                ...
                logger.debug("无查询条件，获取所有已发布资源");
                List<LearningResource> publishedResources = learningResourceMapper.selectByStatus("PUBLISHED");
                List<LearningResource> allResources;
                
                if (fileType != null && !fileType.isEmpty()) {
                    String actualFileType = FILE_TYPE_MAPPING.getOrDefault(fileType, fileType);
                    logger.debug("按文件类型过滤: {}", actualFileType);
                    allResources = publishedResources.stream()
                            .filter(r -> r.getFileType() != null && r.getFileType().contains(actualFileType))
                            .collect(Collectors.toList());
                } else {
                    allResources = publishedResources;
                }
    ...
    private List<LearningResource> performFallbackSearch(String query, String fileType) {
        try {
            ...
            // 获取所有已发布资源
            List<LearningResource> allPublished = learningResourceMapper.selectByStatus("PUBLISHED");
            
            // 关键词搜索
            List<LearningResource> results = allPublished.stream()
                    .filter(resource -> {
    ...
    private Map<String, Object> buildPagedResult(List<LearningResource> resources, int page, int pageSize) {
        ...
        for (LearningResource resource : pageResources) {
            ...
            item.put("uploaderRole", resource.getUploaderRole()); // 使用uploaderRole
            item.put("downloadCount", resource.getDownloadCount() != null ? resource.getDownloadCount() : 0); // 使用实际的下载次数
    ...
    @Override
    @Transactional
    public void updateResource(LearningResource resource) {
        try {
            learningResourceMapper.updateById(resource);
            logger.info("资源更新成功: id={}, title={}", resource.getId(), resource.getTitle());
        } catch (Exception e) {
            logger.error("资源更新失败: {}", e.getMessage(), e);
