...
import com.internship.mapper.ResourceDocumentMapper;
import com.internship.service.ResourceDocumentService;
...
public class ResourceDocumentServiceImpl implements ResourceDocumentService {
    ...
    @Override
    public List<ResourceDocument> findPublished() {
        log.debug("查询已发布的资源文档");
        return resourceDocumentMapper.findPublished();
    }
    ...
    @Override
    public PageResult<ResourceDocument> findTeacherPage(Integer page, Integer pageSize, String title, String status,
                                                         String publisherRole, Long publisherId, String filterType) {
        log.debug("教师端分页查询资源文档，页码: {}, 每页大小: {}, 标题: {}, 状态: {}, 发布人身份: {}, 发布人ID: {}, 筛选类型: {}", 
                page, pageSize, title, status, publisherRole, publisherId, filterType);
        
        List<ResourceDocument> resourceDocuments = resourceDocumentMapper.findTeacherPage(
                (page - 1) * pageSize, pageSize, title, status, publisherRole, publisherId, filterType);
        
        Long total = resourceDocumentMapper.countTeacherResources(title, status, publisherRole, publisherId, filterType);
        int totalPages = (int) Math.ceil((double) total / pageSize);
