...
public interface CategoryWeightMapper {
    ...
    @Update("UPDATE category_weight SET deleted = 1, update_time = NOW()")
    int deleteAll();

    @Delete("DELETE FROM category_weight WHERE deleted = 1")
    int deletePhysicallyAll();

    @Insert("<script>" +
            "INSERT INTO category_weight (category_code, category_name, weight, status, create_time, update_time) VALUES " +
            "<foreach collection='list' item='item' separator=','>" +
            "(#{item.categoryCode}, #{item.categoryName}, #{item.weight}, #{item.status}, #{item.createTime}, #{item.updateTime})" +
            "</foreach>" +
            "</script>")
    int batchInsert(List<CategoryWeight> weights);
}
