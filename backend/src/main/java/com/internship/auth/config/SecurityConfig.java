...
import com.internship.filter.TokenFilter;
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        // 允许登录、注册和刷新令牌接口无需认证
                        .requestMatchers("/api/auth/login", "/api/auth/register", "/api/auth/refresh-token", "/api/auth/password-rules").permitAll()
                        // 允许企业注册相关接口无需认证
                        ...
                        // 允许模板文件下载（学生需要下载模板）
                        .requestMatchers("/api/admin/templates/*/download").permitAll()
                        // 允许 Swagger 文档访问
                        ...
                        // 教师班级接口（所有教师角色都可访问）
                        .requestMatchers("/api/teacher/classes/**").hasAnyRole("TEACHER", "TEACHER_COLLEGE", "TEACHER_DEPARTMENT", "TEACHER_COUNSELOR")
                        // 教师看板接口（所有教师角色都可访问）
                        .requestMatchers("/api/teacher/dashboard/**").hasAnyRole("TEACHER", "TEACHER_COLLEGE", "TEACHER_DEPARTMENT", "TEACHER_COUNSELOR")
                        // 其他所有请求需要认证
