...
import com.internship.entity.GroupTable;
...
public class AdminController {
    ...
    @Autowired
    private com.internship.service.ClassService classService;

    @Autowired
    private com.internship.service.StudentUserService studentUserService;

    @Autowired
    private com.internship.service.TeacherUserService teacherUserService;

    @Autowired
    private com.internship.service.CompanyUserService companyUserService;

    @Autowired
    private com.internship.service.AdminUserService adminUserService;
     ...
     */
    @Autowired
    private com.internship.mapper.ProblemFeedbackMapper problemFeedbackMapper;
    ...
    public Result getRecentFeedback() {
        log.info("获取最近的问题反馈");
        try {
            List<com.internship.entity.ProblemFeedback> feedbackList = problemFeedbackMapper.findRecent(5);
            return Result.success(feedbackList);
        } catch (Exception e) {
            log.error("获取最近的问题反馈失败: {}", e.getMessage());
            return Result.error("获取最近的问题反馈失败");
        }
    }
}