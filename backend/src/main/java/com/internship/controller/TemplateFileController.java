...
import com.internship.service.TemplateFileService;
...
public class TemplateFileController {
    ...
    public ResponseEntity<byte[]> downloadTemplate(@PathVariable Long id) {
        ...
        try {
            byte[] fileBytes = aliyunOSSOperator.downloadFile(template.getFileUrl());
            templateFileService.incrementDownloadCount(id);

            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
            headers.setContentDisposition(ContentDisposition.builder("attachment")
                    .filename(URLEncoder.encode(template.getName(), StandardCharsets.UTF_8.toString()).replaceAll("\\+", "%20"))
                    .build());
