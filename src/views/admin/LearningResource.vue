...
// 获取资源列表
    const fetchResources = async (): Promise<void> => {
      ...
      try {
        ...
        logger.log('API响应数据结构:', response.data)
        ...
        if (response.data && response.data.list) {
          logger.log('使用response.data.list，长度:', response.data.list.length)
          allProcessedResources = response.data.list.map(item => ({
            id: item.id,
          ...
          logger.log('直接使用response.data数组，长度:', response.data.length)
          allProcessedResources = response.data.map(item => ({
            id: item.id,
            title: highlightKeyword(item.title, searchForm.value.title),
            originalTitle: item.title,
            type: normalizeResourceType(item.fileType || item.type || item.resourceType || item.file_category || item.category),
