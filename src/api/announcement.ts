...
// 创建公告阅读记录
export const createAnnouncementReadRecord = (data: { announcementId: number; userId: number; userRole: string }) => {
  return request<ApiResponse<ReadRecord>>({
...
// 根据用户类型和用户信息获取公告列表
export const getAnnouncementsForUser = (userType: string, userInfo: string) => {
  return request<ApiResponse<Announcement[]>>({
