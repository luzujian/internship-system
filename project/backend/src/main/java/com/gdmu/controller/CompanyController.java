...
public class CompanyController {
    ...
    public Result updateCompanyInfo(@RequestBody Map<String, Object> companyInfo) {
        ...
        try {
            ...
            if (companyInfo.containsKey("photos")) {
                ...
                log.info("接收到的 photos 原始值: {}, 长度: {}", photos, photos != null ? photos.length() : 0);
            ...
            if (companyInfo.containsKey("videos")) {
                ...
                log.info("接收到的 videos 原始值: {}, 长度: {}", videos, videos != null ? videos.length() : 0);
    ...
    public Result getStats() {
        ...
        try {
            ...
            List<StudentInternshipStatus> allStatuses = studentInternshipStatusService.list(null, null, null, null, getCurrentCompanyId(), null);
    ...
    public Result getRecentApplications(@RequestParam Long companyId, @RequestParam(defaultValue = "5") Integer limit) {
    ...
    public Result getNotifications(@RequestParam(defaultValue = "5") Integer limit) {
        ...
        try {
            Long companyId = getCurrentCompanyId();
            log.info("当前企业ID: {}", companyId);
            List<CompanyNotification> notifications = companyNotificationService.findByCompanyId(companyId, limit);
            log.info("查询到的通知数量: {}", notifications.size());
            
            List<java.util.Map<String, Object>> result = notifications.stream()
                    .map(notif -> {
                        java.util.Map<String, Object> item = new java.util.HashMap<>();
                        item.put("id", notif.getId());
                        item.put("type", notif.getNotificationType());
                        item.put("title", notif.getTitle());
                        item.put("content", notif.getContent());
                        item.put("time", formatTime(notif.getCreateTime()));
                        item.put("priority", notif.getPriority());
                        item.put("positionId", notif.getPositionId());
                        item.put("positionName", getPositionName(notif.getPositionId()));
                        return item;
                    })
                    .toList();
