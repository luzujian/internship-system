...
# 前提：已配置 SSH 密钥免密登录（ssh-copy-id root@8.148.216.84）
# ============================================================
...
SSH_OPTS=(-o Port="$SSH_PORT" -o StrictHostKeyChecking=accept-new)
...
# ── 步骤 3: 传输文件 ──
info "传输前端文件到服务器..."
scp "${SSH_OPTS[@]}" -r "$FRONTEND_DIR/dist/"* "$SSH_HOST:$DIST_TARGET" 2>&1
ok "前端文件传输完成"

info "传输后端 jar 到服务器..."
scp "${SSH_OPTS[@]}" "$JAR_SRC" "$SSH_HOST:$JAR_TARGET" 2>&1
ok "后端 jar 传输完成"

# ── 步骤 4: 重启容器 ──
info "重启 Docker 容器..."
ssh "${SSH_OPTS[@]}" "$SSH_HOST" "cd $SERVER_DIR && docker compose restart backend frontend"
ok "容器重启指令已发送"

# ── 步骤 5: 等待健康检查 ──
info "等待后端启动..."
sleep 15
HEALTH=$(ssh "${SSH_OPTS[@]}" "$SSH_HOST" "docker ps --format '{{.Names}} {{.Status}}' | grep internship")
echo "$HEALTH"
ok "部署完成！"
